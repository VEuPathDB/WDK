...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		multiSelect: function(o, callback) {
			...
			$(this).each( function() {
				var select = $(this);
				var html = '<input type="hidden" class="multiSelect" commandUrl="' + $(select).attr('commandUrl') + '" id="' + $(select).attr('id') + '" value=""/>';
				html += '<input type="text" readonly="readonly" class="multiSelect display" value="" style="cursor: default;" />';
				...
				// Events
				$(select).next('.multiSelect').next('.multiSelect.display').mouseover( function() {
					$(this).addClass('hover');
				}).mouseout( function() {
					...
					if( $(this).hasClass('active') ) {
						$(this).multiSelectOptionsHide(callback);
					} else {
						$(this).multiSelectOptionsShow(callback);
					}
					return false;
				}).focus( function() {
				...
				if( o.selectAll ) {
					var sa = true;
					$(select).next('.multiSelect').next('.multiSelect.display').next('.multiSelectOptions').find('INPUT:checkbox').not('.selectAll').each( function() {
						if( !$(this).attr('checked') ) sa = false;
					});
					if( sa ) $(select).next('.multiSelect').next('.multiSelect.display').next('.multiSelectOptions').find('INPUT.selectAll').attr('checked', true).parent().addClass('checked');
				}
				
				// Handle Select All
				$(select).next('.multiSelect').next('.multiSelect.display').next('.multiSelectOptions').find('INPUT.selectAll').click( function() {
				...
				// Handle checkboxes
				$(select).next('.multiSelect').next('.multiSelect.display').next('.multiSelectOptions').find('INPUT:checkbox').click( function() {
				...
				// Initial display
				$(select).next('.multiSelect').next('.multiSelect.display').next('.multiSelectOptions').each( function() {
					$(this).multiSelectUpdateSelected(o);
					$(this).find('INPUT:checked').parent().addClass('checked');
				});
				
				// Handle hovers
				$(select).next('.multiSelect').next('.multiSelect.display').next('.multiSelectOptions').find('LABEL').mouseover( function() {
				...
				// Keyboard
				$(select).next('.multiSelect').next('.multiSelect.display').keydown( function(e) {
					// Is dropdown visible?
		...
		// Show the dropdown
		multiSelectOptionsShow: function(callback) {
			// Hide any open option boxes
			$('.multiSelect.display').multiSelectOptionsHide();
			...
			$(this).next('.multiSelectOptions').hover( function() {
				...
				timer = setTimeout('jQuery(multiSelectCurrent).multiSelectOptionsHide(' + callback + '); $(multiSelectCurrent).unbind("hover");', 250);
			});
		...
		multiSelectUpdateSelected: function(o) {
			...
			if( i == 0 ) {
				$(this).prev('INPUT.multiSelect.display').val( o.noneSelected );
				$(this).prev('INPUT.multiSelect.display').prev('INPUT[type=hidden].multiSelect').val( '' );
			} else {
				// Update value (hidden input
				var value=''
				$(this).find('INPUT:checkbox:checked').each( function() {
					if( $(this).parent().text() != o.selectAllText ) value = value + $(this).val() + ',';
				});
				value = value.substr(0, value.length - 1);
				$(this).prev('INPUT.multiSelect.display').prev('INPUT[type=hidden].multiSelect').val( value );
				// Update display (readonly text input w/ pulldown arrow)
				if( o.oneOrMoreSelected == '*' ) {
					...
					$(this).prev('INPUT.multiSelect.display').val( display );
				} else {
					$(this).prev('INPUT.multiSelect.display').val( o.oneOrMoreSelected.replace('%', i) );
				}
			}
		},
