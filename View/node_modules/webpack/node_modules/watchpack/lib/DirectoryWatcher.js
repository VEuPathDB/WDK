		...
		if(mtime) {
			if(this.watchers[path]) {
				this.watchers[path].forEach(function(w) {
					var startTime = w.startTime && Math.floor(w.startTime / FS_ACCURENCY) * FS_ACCURENCY;
					if(!initial || (w.startTime && startTime <= mtime)) {
						w.emit("change", mtime);
					}
				});
			}
		}
	} else if(mtime && type !== "add") {
		if(this.watchers[path]) {
			this.watchers[path].forEach(function(w) {
				w.emit("change", mtime);
			});
...
DirectoryWatcher.prototype.createNestedWatcher = function(path) {
	this.directories[path] = watcherManager.watchDirectory(path);
