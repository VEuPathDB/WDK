...
<c:choose>
  <c:when test="${wdkUser == null || wdkUser.guest}">
    <c:set var="clickEvent" value="wdk.stratTabCookie.setCurrentTabCookie('application', 'basket'); wdk.user.login(wdk.webappUrl('showApplication.do'));" />
...
<c:choose>
  <c:when test="${wdkUser eq null or wdkUser.guest}">
      <a id="mybasket" onclick="wdk.user.login('wdk.webappUrl('showFavorite.do'));"
