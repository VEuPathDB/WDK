...
var renderPrimaryPublication = function(publication) {
  var pubmedLink = _.find(publication, { name: 'pubmed_link' });
  return formatLink(pubmedLink, { newWindow: true });
};

var renderPrimaryContact = function(contact, institution) {
...
var Graphs = React.createClass({
  ...
  _renderGraph(graph, index) {
    ...
    var url = `/cgi-bin/dataPlotter.pl?type=${g.module.value}&project_id=${g.project_id.value}&dataset=${g.dataset_name.value}&template=1&fmt=png&id=${g.graph_ids.value}`;
    return (
...
function recordComponentResolver(recordClassName, DefaultComponent) {
  switch (recordClassName) {
    case "DatasetRecordClasses.DatasetRecordClass": return DatasetRecord;
    default: return DefaultComponent;
  }
}

// format is {text}({link})
function formatLink(link, { newWindow = false } = {}) {
