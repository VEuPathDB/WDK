...
public class StrategyAttributeRemoteHandler implements RemoteHandler {

    private static final String PROP_ATTRIBUTES = "attributes";

    private static final Logger logger = Logger.getLogger(StrategyAttributeRemoteHandler.class);
    ...
    private int cacheResults(JSONObject jsAnswer, QueryInfo queryInfo,
        ...
        try {
            ...
            for (; row < jsRecords.length(); row++) {
                ...
                JSONArray jsValues = jsRecord.getJSONArray("attributes");
                for (int i = 0; i < jsAttributes.length(); i++) {
