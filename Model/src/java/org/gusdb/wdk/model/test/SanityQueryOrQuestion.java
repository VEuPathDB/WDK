...
public abstract class SanityQueryOrQuestion extends WdkModelBase {

    protected String twoPartName;

    /**
     * The parameters that will be used when running this query or question. The
     * keys of the hashtable are the names of the parameters and the values are
    ...
    public SanityParam getParam(String paramName) throws WdkUserException {
        ...
        if (param == null)
            throw new WdkUserException("Sanity" + getTypeCap() + " " + getRef()
                    + " does not include param " + paramName);
        return param;
    }
    ...
    /**
     * @param return
     *        An array of all sanity params provided in the sanity test. Note
     *        that the calling method needs to extract names and values from
     *        each Sanity Param object.
     */
     ...
     * 
     * @param return
     *        a Hashtable where the keys are the name of each parameter and the
     *        values are the actual values of the parameter.
     */
    ...
    public String toString() {
        StringBuffer result = new StringBuffer("Sanity" + getTypeCap()
                + " twoPartName = " + twoPartName + " minOutputLength = "
                + minOutputLength + " maxOutputLength = " + maxOutputLength);
    ...
    public abstract String getCommand(String globalArgs)
            throws WdkModelException;

    protected String getCommand(String globalArgs, String firstPart)
            throws WdkModelException {
        ...
        if (params != null) {
            ...
            for (int i = 0; i < params.length; i++) {
                ...
                command.append(nextParam.getName() + " \""
                        + nextParam.getValue() + "\" ");
            }
        }
