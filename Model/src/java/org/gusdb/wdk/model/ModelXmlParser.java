 ...
 * 
 * <p>
 * First, the parser will start from the top level model file, and get sub-model
 * files from the {@code <import>} tag. currently, we only support
 * {@code <import>} in the top level model file, and sub-model file cannot have
 * {@code <import>} to other sub-models.
 * </p>
 * 
 * <p>
 * Then the top level model and each such-model's XML file will be validated
 * against {@code wdkModel.rng}, and if validation fails, an exception will be
 * thrown out with the line number where it failed.
 * </p>
 * 
 * <p>
 * Nest, the parser will first get global constants defined in the top level
 * model, and substitute them into the text of the model; then it will read
 * constants from each sub-model, and those constants can override the global
 * constants in the scope of the sub-model; and the constants are substituted
 * into the text of the sub-models.
 * </p>
 * 
 * <p>
 * In the next step, the parser will substitute the macros defined in model.prop
 * into the text of the top level model and each sub-model.
 * </p>
 * 
 * <p>
 * Next, a DOM will be created for each top level and sub-model, and DOM of
 * sub-model will be injected into top level model. the {@code <wdkModel>} root
 * tag in each sub-model will be ignored, but all its child elements will be
 * inserted into the {@code <wdkModel>} in the top level DOM.
 * </p>
 * 
 * <p>
 * Next, the DOM will be streamed into Apache
 * {@link org.apache.commons.digester.Digester}, and a {@link WdkModel} object
 * will be created.
 * </p>
 * 
 * <p>
 * Next, {@link WdkModel#excludeResources} will be called, and the resources
 * that don't belong to the current project will be removed, and the model
 * objects will be cleaned up to remove excluded resources, and some of the
 * unique constraints will be applied here. The include/exclude projects flag
 * are used to declared if a resource belongs, or doesn't belong, to a list of
 * projects.
 * </p>
 * 
 * <p>
 * Lastly, {@link WdkModel#resolveReferences} will be called and object
 * references will be resolved, for example, a {@link Question} object will have
 * a reference to {@link RecordClass} object. In certain cases, a clone will be
 * created, such as a {@link Question will have a copy of the ID {@link Query},
 * and a {@link Query} will have a local copy of all the {@link Param}s because
 * the properties of a {@link Param} can be overriden in the {@code <paramRef>}
 * inside of {@code <question>, <sqlQuery>, and <processQuery>}.
 * </p>
 * 
 * @author jerric
 * 
...
public class ModelXmlParser extends XmlParser {
  ...
  public WdkModel parseModel(String projectId)
    ...
    // load property map
    Map<String, String> properties = loadProperties(projectId, modelPropURL,
        config);

    // load master model
    logger.debug("Resolving WDK model...");
    Set<String> replacedMacros = new LinkedHashSet<String>();
    Document masterDoc = buildMasterDocument(projectId, modelURL, properties,
        replacedMacros);
  ...
  /**
   * Before building the master model DOM, the constants will be loaded first,
   * and then substituted into each sub model dom. Then each sub model dom will
   * be combined into the master model. At last, the properties from model.prop
   * will be substituded into the master model.
   * 
   * @param projectId
   * @param wdkModelURL
   * @param properties
  ...
  private Document buildMasterDocument(String projectId, URL wdkModelURL,
    ...
    Document masterDoc = loadDocument(wdkModelURL, properties, replacedMacros,
        constants);
    ...
    for (int i = 0; i < children.getLength(); i++) {
      ...
      Document importDoc = loadDocument(importURL, properties, replacedMacros,
          subConsts);
  ...
  private void configureModel(Digester digester) {
    ...
    configureNode(digester, "wdkModel/sampleStratsAuthor", SampleStratsAuthorFilter.class, "setSampleStratsAuthor");
    ...
    configureNode(digester, "wdkModel/defaultPropertyList", PropertyList.class,
        "addDefaultPropertyList");
  ...
  private void configureRecordClassSet(Digester digester) {
    // record class set
    configureNode(digester, "wdkModel/recordClassSet", RecordClassSet.class,
        "addRecordClassSet");
    ...
    digester.addCallMethod(
        "wdkModel/recordClassSet/recordClass/reporter/property", "setValue", 0);
    ...
    // filter instances
    configureNode(digester, "wdkModel/recordClassSet/recordClass/answerFilter",
    ...
    configureNode(digester,
        ...
        WdkModelText.class, "addDescription");
    digester.addCallMethod(
        "wdkModel/recordClassSet/recordClass/table/description", "setText", 0);
    ...
    configureNode(digester, "wdkModel/recordClassSet/recordClass/nestedRecord",
    ...
    configureNode(digester, "wdkModel/recordClassSet/recordClass/recordView",
  ...
  private void configureQuerySet(Digester digester) {
    ...
    // defaultTestParamValues
    configureParamValuesSet(digester,
        "wdkModel/querySet/defaultTestParamValues", "addDefaultParamValuesSet");
    ...
    // testParamValues
    configureParamValuesSet(digester,
        "wdkModel/querySet/sqlQuery/testParamValues", "addParamValuesSet");
    ...
    configureNode(digester, "wdkModel/querySet/sqlQuery/column", Column.class,
        "addColumn");
    ...
    // testParamValues
    configureParamValuesSet(digester,
        "wdkModel/querySet/processQuery/testParamValues", "addParamValuesSet");
  ...
  private void configureQuestionSet(Digester digester) {
    ...
    // question
    configureNode(digester, "wdkModel/questionSet/question", Question.class,
        "addQuestion");
    ...
    digester.addCallMethod("wdkModel/questionSet/question/summary", "setText",
        0);
    ...
    configureNode(digester, "wdkModel/questionSet/question/dynamicAttributes",
    ...
    configureNode(digester,
        ...
        WdkModelText.class, "addDescription");
    digester.addCallMethod(
        "wdkModel/questionSet/question/summaryView/description", "setText", 0);
  }

  private void configureXmlQuestionSet(Digester digester) {
    // load XmlQuestionSet
    configureNode(digester, "wdkModel/xmlQuestionSet", XmlQuestionSet.class,
        "addXmlQuestionSet");
    ...
    configureNode(digester, "wdkModel/xmlQuestionSet/xmlQuestion/description",
        WdkModelText.class, "addDescription");
    digester.addCallMethod("wdkModel/xmlQuestionSet/xmlQuestion/description",
        "setText", 0);
  }
  ...
  private void configureAttributeFields(Digester digester) {
    ...
    configureNode(digester, "*/columnAttribute", ColumnAttributeField.class,
        "addAttributeField");
    ...
    configureNode(digester, "*/linkAttribute/displayText", WdkModelText.class,
        "addDisplayText");
