    ...
    for (AnswerFilterInstanceReference reference : referenceList) {
      ...
      instanceMap.put(ref, reference.getInstance());
			...
			if (this.name.equals("gene_filters")) {	
					...
					String[] filterName = ref.split("_");
					String abbrev = filterName[0];
					//logger.debug("\n\n\n" + abbrev + "====" + ref + "\n\n\n");

					// reading Family name from the displayName in the model, generated by gene filter injector
					// and defined in the template ApiCommonShared/Model/lib/dst/geneFilterTemplates.dst
					...
					if (ref.contains("instances") && !ref.contains("distinct")) { 
							if (!instanceCountMap.containsKey(abbrev))  instanceCountMap.put(abbrev, 1);
							else instanceCountMap.put(abbrev, instanceCountMap.get(abbrev)+1);
