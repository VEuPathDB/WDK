...
public class ModelCacher extends BaseCLI {
  ...
  public void dropTables(WdkModel wdkModel, String schema) {
    ...
    for (String sequence : sequences) {
      try {
        SqlUtils.executeUpdate(dataSource,
            "DROP SEQUENCE " + schema + sequence, "wdk-drop-sequence");
      } catch (Exception ex) {
        ex.printStackTrace();
      }
    }
    ...
    for (String table : tables) {
      try {
        SqlUtils.executeUpdate(dataSource, "DROP TABLE " + schema + table,
            "wdk-drop-table");
      } catch (Exception ex) {
        ex.printStackTrace();
      }
  ...
  private void createParamTable(WdkModel wdkModel, DataSource dataSource,
    ...
    // create table
    sql = "CREATE TABLE " + schema + "wdk_params ( "
  ...
  private void saveQuestion(Question question, PreparedStatement psQuestion,
    ...
    int questionId = platform.getNextId(userDb.getDataSource(),
        schemaWithoutDot, "wdk_questions");
  ...
  private void saveParam(WdkModel wdkModel, Question question, Param param,
      int questionId, PreparedStatement psParam, PreparedStatement psEnum,
    ...
    int paramId = database.getPlatform().getNextId(database.getDataSource(),
        schemaWithoutDot, "wdk_params");
  ...
  private void saveEnums(Question question, AbstractEnumParam param,
      int paramId, PreparedStatement psEnum) throws WdkModelException,
      SQLException {
