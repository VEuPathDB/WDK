...
/**
 * This class is used to parse a boolean expression and get the step ids and
 * the boolean operators from it.
 * 
 * @author Jerric
 * @created Sep 14, 2005
 */
public class BooleanExpression {
  ...
  private String replaceLiterals(String expression, Map<String, String> replace)
      throws WdkModelException {
  ...
  private Step parseBlock(String block, Map<String, String> replace,
    ...
    if (triplet.length == 1) { // only remove one pair of parentheses
      ...
      return user.createBooleanStep(left, right, operator, useBooleanFilter,
          filterName);
    }
  }
  ...
  /**
   * Split the block into left operand, operator, right operand. If it cannot be
   * divided into three part, there should be only one part, put in [0].
   * 
   * @param block
   * @return
   */
