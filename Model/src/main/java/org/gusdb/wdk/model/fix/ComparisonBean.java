 ...
 * later display in a tabular report.
 * 
 * @author crisl-adm
 */
public class ComparisonBean {
  ...
  public ComparisonBean(String context, String missingFromProdName, String missingFromQaName,
      List<String> missingFromProd, List<String> missingFromQa) {
    _context = context;
  ...
  public String toString() {
    return NL + getContext() + ":" + NL + getMissingFromQaName() + "-" +
        Arrays.toString(getMissingFromQa().toArray()) + NL + getMissingFromProdName() + " - " +
   ...
   * 
   * @param displayOption
   *          - invalid, new or other. If invalid show only invalid questions (i.e., missing from qa) and if
   *          new show only new questions (i.e., missing from prod). Otherwise show both side by side.
   */
  public void display(String displayOption) {
    ...
    switch (displayOption) {
      ...
      default:
        ...
        if (getMissingFromQa().isEmpty() && getMissingFromProd().isEmpty())
          return;
        ...
        Optional<String> missingFromQaLongestString = getMissingFromQa().stream().max(
            Comparator.comparingInt(String::length));
        int missingFromQaLength = missingFromQaLongestString.isPresent()
            ? Math.max(getMissingFromQaName().length(), missingFromQaLongestString.get().length())
            : getMissingFromQaName().length();
        Optional<String> missingFromProdLongestString = getMissingFromProd().stream().max(
            Comparator.comparingInt(String::length));
        int missingFromProdLength = missingFromProdLongestString.isPresent()
            ? Math.max(getMissingFromProdName().length(), missingFromProdLongestString.get().length())
            : getMissingFromProdName().length();

        // Create the table header
        String line = new String(new char[5 + missingFromQaLength + missingFromProdLength]).replace('\0',
            '-');
        System.out.println(line);
        System.out.printf("%s|%s|%n", StringUtils.center(getMissingFromQaName(), missingFromQaLength + 2),
  ...
  public void display(String name, List<String> items) {
    ...
    int columnLength = itemsLongestString.isPresent()
        ? Math.max(name.length(), itemsLongestString.get().length()) : name.length();
