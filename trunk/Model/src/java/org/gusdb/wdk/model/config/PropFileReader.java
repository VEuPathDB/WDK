 ...
 * the required properties. WDK's model.prop is not read by this reader.
 * 
 * TODO - Maybe this file is only used by OrthoMCL, consider moving it into that
 * project.
 * 
 * @author jerric
 * 
    ...
    try {
      ...
      for (String key : getRequiredProps()) {
        if (!props.containsKey(key)) {
          throw new IOException("Proteome config file missing property [ "
              + key + "].");
        }
      }
