 ...
 *
 * Provides a unified command line interface for writing
 * the results of a Reporter to files.
 */
public class Dumper {
	...
	if (cmdLine.hasOption("params")) questionParams = cmdLine.getOptionValues("params");
	String[] reporterConfig = new String[0];
	if (cmdLine.hasOption("config")) reporterConfig = cmdLine.getOptionValues("config");
    ...
    static void addListOption(Options options, String argName,
    ...
    static Options declareOptions() {
        ...
        // params
	addListOption(options, "params", "space delimited list of param_name param_value ....", false);

        // reporter config
	addListOption(options, "config", "space delimited list of property_name property_value ....", false);
	...
	throws WdkModelException, NoSuchAlgorithmException, SQLException,
        ...
        if (inputValues.length % 2 != 0) {
            throw new IllegalArgumentException(
                    "The -" + argName + " option must be followed by key value pairs only");
        }
