...
public class ParamGroup {
  ...
  private static final Set<String> TRUE_VALUES = new HashSet<>(
      Arrays.asList(new String[] { "true", "1", "yes" })
  );
  ...
  /**
   * Looks for the key in the parameter set. If there is no value present, returns an empty string.
   * 
   * @param key
   *          parameter name
   * @return the value of the parameter or an empty string if no value exists
   * @throws IllegalArgumentException
   *           if multiple values exist for this parameter
   */
  ...
  /**
   * Looks for the key in the parameter set. If there is no value present, returns the passed default value.
   * 
   * @param key
   *          parameter name
   * @param defaultValue
   *          value returned if param not present
  ...
  public String getValueOrDefault(String key, String defaultValue) {
    ...
    if (values != null && values.length > 1) {
      throw new IllegalArgumentException("The key [ " + key +
          " ] contains multiple values which cannot be retrieved with this method.");
    }
  ...
  public Long getLongValue(String key) {
    checkValidKey(key);
    if (!_defs.get(key).getDataType().equals(ParamDef.DataType.LONG)) {
      throw new IllegalArgumentException("The key [ " + key + " ] does not refer to a long param.");
    }
    String str = getValue(key);
