 ...
 * 
 * @author rdoherty Though mostly copied from example code found here:
...
public class AuthenticationService {
  ...
  public String authRequest(String userSuppliedString, String webAppBaseUrl) {
    try {
       ...
       * 
       * // always use old v1.x method of url redirection if (true) { // Option 1: GET HTTP-redirect to the
       * OpenID Provider endpoint // The only method supported in OpenID 1.x // redirect-URL usually limited
       * ~2048 bytes httpResp.sendRedirect(authReq.getDestinationUrl(true)); return null; } else { // Option
       * 2: HTML FORM Redirection (Allows payloads >2048 bytes) RequestDispatcher dispatcher =
       * getServletContext().getRequestDispatcher("formredirection.jsp"); httpReq.setAttribute("parameterMap",
       * authReq.getParameterMap()); httpReq.setAttribute("destinationUrl", authReq.getDestinationUrl(false));
  ...
  public OpenIdUser verifyResponse(String requestUrl, String queryString, Map<String, String[]> params) {
    try {
      ...
      VerificationResult verification = _manager.verify(receivingURL.toString(), response,
          _discoveryInformation);
