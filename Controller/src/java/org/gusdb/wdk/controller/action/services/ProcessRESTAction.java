...
public class ProcessRESTAction extends Action {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form,
    ...
    try {
      ...
      if (qFullName != null) wdkQuestion = wdkModel.getQuestion(qFullName);
      if (wdkQuestion == null)
      ...
      for (String key : paramMap.keySet()) {
        ...
        if (param instanceof DatasetParamBean) {
          ...
          logger.debug("dataset data: '" + rawValue + "'");
      ...
      StepBean step = wdkUser.createStep(wdkQuestion, params, null, false,
          true, Utilities.DEFAULT_WEIGHT);
      ...
      if (ex instanceof WdkModelException && outputType != null) {
        ...
        if (wdkEx.getParamErrors() != null) {
          ...
          reportError(response, exMap, "Output Parameter Error", "011",
              outputType);
        }
  ...
  private void writeWADL(QuestionBean wdkQuestion, PrintWriter writer)
    ...
    writer.println("<doc title='summary'><![CDATA[" + wdkQuestion.getSummary()
        + "]]></doc>");
    ...
    for (String key : params.keySet()) {
      ...
      writer.println("<param name='" + key + "' type='xsd:string' required='"
      ...
      writer.println("<doc title='help'><![CDATA[" + param.getHelp()
          + "]]></doc>");
      writer.println("<doc title='default'><![CDATA[" + param.getDefault()
          + "]]></doc>");

      if (param instanceof EnumParamBean) {
        EnumParamBean ep = (EnumParamBean) param;
        if (ep.getMultiPick()) writer.println("<doc title='MultiValued'>"
            + "Provide one or more values. " + "Use comma as a delimter.</doc>");
    ...
    for (String tab : wdkQuestion.getReportMakerTablesMap().keySet())
      writer.println("<option value='" + tab
          + "'><doc title='description'><![CDATA["
